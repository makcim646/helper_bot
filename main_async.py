...
async def process_callback_button1(callback_query: types.CallbackQuery):
    ...
    await bot.send_message(callback_query.from_user.id, port_login(port_out), reply_markup=otvet2)
...
@dp.callback_query_handler(lambda c: c.data == 'see_olt')
async def process_callback_button1(callback_query: types.CallbackQuery):
    port_out = callback_query.message.text.split('\n')[-1]
    await bot.answer_callback_query(callback_query.id)
    await bot.send_message(callback_query.from_user.id, 'Проверяю порт OLT\nВремя на проверку ~ 8мин.')
    await bot.send_message(callback_query.from_user.id, see_olt_port(port_out))
